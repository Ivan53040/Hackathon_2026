...
import { buildArena, buildSimpleArena, GAP, type ArenaRefs } from './arena';
...
import { getGraphicsQuality } from './quality';
...
let meHotColor = '';
let themHotColor = '';
let sceneKind: 'roman' | 'moon' | 'simple' = 'roman';
...
export function initView(overlayCanvas: HTMLCanvasElement): void {
  ...
  sceneKind = getGraphicsQuality() === 'low'
    ? 'simple'
    : requestedScene === 'moon' ? 'moon' : 'roman';
  const lowQuality = sceneKind === 'simple';
  gl.dataset.scene = sceneKind;

  renderer = new THREE.WebGLRenderer({ canvas: gl, antialias: !lowQuality });
  /*
   ...
   */
  const MAX_PIXEL_RATIO = lowQuality ? 1 : 1.5;
  renderer.setPixelRatio(Math.min(devicePixelRatio, MAX_PIXEL_RATIO));
  renderer.setSize(innerWidth, innerHeight, false);
  renderer.outputColorSpace = THREE.SRGBColorSpace;
  renderer.toneMapping = sceneKind === 'roman' ? THREE.ACESFilmicToneMapping : THREE.NoToneMapping;
  renderer.toneMappingExposure = sceneKind === 'roman' ? 1.08 : 1;
  renderer.shadowMap.enabled = sceneKind === 'roman';
  renderer.shadowMap.type = THREE.PCFSoftShadowMap;

  scene = new THREE.Scene();
  fps = new FpsCamera(innerWidth / innerHeight);
  arena = sceneKind === 'roman'
    ? buildRomanArena(scene)
    : sceneKind === 'simple' ? buildSimpleArena(scene) : buildArena(scene);
  if (sceneKind === 'roman') buildCrowd(scene);
  if (sceneKind === 'moon') buildScenery(scene);
  ...
  if (!lowQuality) initPostFx(renderer, scene, fps.cam);
...
function onResize(): void {
  renderer?.setSize(innerWidth, innerHeight, false);
  if (renderer && sceneKind !== 'simple') resizePostFx(renderer);
...
export function renderView(s: MatchState, f: WandFrame, dt: number): void {
  if (!renderer) return;
  if (f.source !== 'pen' && s.winner === null) touchWebcamPip();
  else pauseWebcamPip();
  clock += dt;
  ...
  if (sceneKind === 'simple' || !renderPostFx()) renderer.render(scene, fps.cam);
...
export function disposeView(): void {
  ...
  if (sceneKind === 'roman') { disposeRomanArena(); disposeCrowd(); }
  else if (sceneKind === 'moon') disposeScenery();
  disposeWebcamPip();
  disposePostFx();
  disposeScene();
  renderer?.dispose();
