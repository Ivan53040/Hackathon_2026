...
> **v5 — 第一人稱 + 臉部追蹤。** 印出來貼在牆上，每個 Block 結束時全隊站著唸一次。
...
- [ ] 逐行唸 `core/types.ts`（★ `WandFrame.head` / `bodyScale`；`Projectile.toX`）
...
### ✅ M0（H+2）
- [ ] **所有人開網頁移動滑鼠，console 印出 WandFrame，`head` 會跟著變**
...
### C · match
- [ ] ★ `x` 由 `WandFrame.head` 驅動，**無慣性無加速度**
- [ ] 快擊 `bolt` + 投射物 + 命中判定（比對 `toX`）
- [ ] **學徒 bot**
...
### Ivan
- [ ] 🔴 ★ `faceSource.ts`：`head` + `bodyScale`（外眼角距離）
- [ ] 🔴 ★ **融合**：`tip` 換算成相對身體 → 玩家挪位置符文不會變形
- [ ] ★ `head` 用**更重的平滑**（頭抖 → 畫面暈）
- [ ] 🔴 **MediaPipe wasm + .task 下載到 `public/mediapipe/`**（會場斷網保命）
- [ ] ★ 量 fps：兩個模型同時跑掉幀就把 face 降到 15Hz
...
- [ ] 1. **face 降到 15Hz** → 再不行**砍掉 faceSource**，`head` 改用滑鼠或 A/D
- [ ] 2. **砍掉重擊 ⬠，只剩 △** —— 辨識率接近 100%，遊戲變純反應速度，仍然成立
- [ ] 3. **相機視差關掉** —— 解掉暈眩風險
- [ ] 4. **貼綠色膠紙**（若手部追蹤不穩），包裝成「魔杖的能量水晶」
